...
class TherapyPlugin(spanreed.plugin.Plugin):
    def __init__(self, todoist_api: Todoist, *args, **kwargs):
        # TODO: refactor to be per-user
        self.todoist_api = todoist_api
        super().__init__(name="Therapy", *args, **kwargs)
    ...
    async def run(self):
        ...
        while True:
            ...
            tasks: List[Task] = await self._todoist_api.get_tasks_with_tag(tag)
            if len(tasks) != 1:
                raise RuntimeError(f'Expected exactly one task with the tag {tag}, got {len(tasks)}')
            task, = tasks
            comment = await self._todoist_api.get_first_comment_with_yaml(task)
            ...
            if date_str not in dates:
                ...
                await self._todoist_api.update_comment(comment, content=new_comment_content)
                await self._todoist_api.update_task(task, content=new_task_content)
                await self._todoist_api.set_due_date_to_today(task)
