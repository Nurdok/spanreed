...
class UserData:
    ...
    async def create(cls, user_id: int, redis_api: redis.Redis) -> "UserData":
        ...
        self.event_storage = await EventStorageRedis.create(
            user_id=self.user_id, redis_api=redis_api
        )
        return self
...
async def ask_journal(context: ContextTypes.DEFAULT_TYPE):
    user_data: UserData = context.application.user_data[context.job.user_id][
        UserData.__name__
    ]
    activity_type: ActivityType = ActivityType.JOURNAL

    if (
        event_type := user_data.event_storage.find_event(
            ActivityType.JOURNAL, datetime.date.today()
        )
    ) is not None:
        logger.info(
...
def remove_job_if_exists(name: str, job_queue: telegram.ext.JobQueue):
...
async def subscribe_existing_users_on_startup(
    ...
    for user_id in user_ids:
        ...
        application.user_data[user_id].setdefault(
            UserData.__name__,
            await UserData.create(user_id=user_id, redis_api=redis_api),
        )
